...
- 能力库扫描门禁：技术栈确认前，先把项目所需技术能力映射到成熟、开源、维护度高的第三方库，同时审查仓库形态，并让用户确认“核心栈 + 库清单”。
- Product MVP UI 质量门禁：进入前端代码前，先记录设计判断、上下文相关的设计刻度、设计系统和交互状态，并执行反模板检查；不会把 landing page 的 taste 规则或默认图标/组件库硬套到后台、数据表或产品工作台。
